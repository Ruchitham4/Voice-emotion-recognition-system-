"""
This script builds 2 additional actor folders (25 and 26) using features from the
...
import shutil
import random
from config import TRAINING_FILES_PATH, TESS_ORIGINAL_FOLDER_PATH
...
class TESSPipeline:
    ...
    def create_tess_folders(path):
        """
        Creates Actor_25 and Actor_26 folders if they don't exist,
        then copies and renames TESS dataset files accordingly.
        """
        ...
        for subdir, _, files in os.walk(path):
            for filename in files:
                ...
                # Determine the correct emotion label
                for key, value in label_conversion.items():
                    if base.endswith(value):
                        # Generate random numbers for renaming
                        random_list = random.sample(range(10, 99), 7)
                        file_name = '-'.join([str(i) for i in random_list])
                        file_name_with_correct_emotion = f"{file_name[:6]}{key}{file_name[8:]}{extension}"
                        ...
                        try:
                            ...
                            print(f"Error copying {old_file_path} to {new_file_path}: {e}")


if __name__ == '__main__':
    print("Starting TESSPipeline...")
    TESSPipeline.create_tess_folders(TESS_ORIGINAL_FOLDER_PATH)
    print("TESSPipeline execution completed.")
